...
:   URL to the Prometheus metrics server (only needed if enable-metrics is set).
...
:   If set to True, include usage on every request (even when stream_options is not specified)
