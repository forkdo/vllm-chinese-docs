...
:   单个输入文件的路径或 URL。目前支持本地文件路径或 HTTP 协议（http 或 https）。如果指定 URL，文件应可通过 HTTP GET 访问。

#### `-o`, `--output-file`

:   单个输出文件的路径或 URL。目前支持本地文件路径或 Web（http 或 https）URL。如果指定 URL，文件应可通过 HTTP PUT 访问。

#### `--output-tmp-dir`

:   上传输出文件到输出 URL 之前存储输出文件的目录。
...
:   Prometheus 指标服务器的 URL（仅在启用指标时需要）。

:   默认值：`0.0.0.0`

#### `--port`

:   Prometheus 指标服务器的端口号（仅在启用指标时需要）。
...
:   如果设置为 True，则在每个请求中包含 usage（即使未指定 stream_options）
...
:   可能的选项：`auto`、`classify`、`embed`、`mm_encoder_only`、`none`、`reward`
...
:   可能的选项：`auto`、`deepseek_v32`、`hf`、`mistral`、`slow`
...
:   是否始终使用 eager 模式的 PyTorch。如果为 True，我们将禁用 CUDA 图并始终在 eager 模式下执行模型。如果为 False，我们将混合使用 CUDA 图和 eager 执行以获得最大性能和灵活性。
...
:   当 `SamplingParams` 中指定 `logprobs` 时，返回的最大对数概率数。默认值来自 OpenAI Chat Completions API 的默认值。-1 表示无限制，即允许返回所有（output_length * vocab_size）个对数概率，这可能导致 OOM。
...
是否禁用滑动窗口。如果为 True，我们将禁用模型的滑动窗口功能，将其限制在滑动窗口大小内。如果模型不支持滑动窗口，则此参数将被忽略。

:   默认值：`False`

#### `--disable-cascade-attn`, `--no-disable-cascade-attn`

:   为 V1 禁用级联注意力。虽然级联注意力不会改变数学正确性，但禁用它可以防止潜在的数值问题。请注意，即使此参数设置为 False，级联注意力也仅会在启发式判断有益时才会使用。

:   默认值：`False`

#### `--skip-tokenizer-init`, `--no-skip-tokenizer-init`

:   跳过分词器和去分词器的初始化。期望输入中包含有效的 `prompt_token_ids` 且 prompt 为 `None`。生成的输出将包含 token ID。
...
:   如果为 `True`，则允许通过 `prompt_embeds` 键传递文本嵌入作为输入。
    ...
    警告：如果传入的嵌入形状不正确，vLLM 引擎可能会崩溃。仅对可信用户启用此标志！

:   默认值：`False`

#### `--served-model-name`

:   API 中使用的模型名称。如果提供了多个名称，服务器将响应其中任意一个名称。响应中 model 字段的模型名称将是此列表中的第一个名称。如果未指定，模型名称将与 `--model` 参数相同。请注意，此名称（或名称列表）也将用于 Prometheus 指标的 `model_name` 标签内容中；如果提供了多个名称，指标标签将采用第一个名称。
...
:   如果为字典，则包含要转发给 Hugging Face 配置的参数。如果为可调用对象，则用于更新 HuggingFace 配置。

:   默认值：`{}`

#### `--pooler-config`

:   池化器配置，用于控制池化模型中的输出池化行为。
...
:   生成配置的文件夹路径。默认为 `"auto"`，将从模型路径加载生成配置。如果设置为 `"vllm"`，则不加载生成配置，将使用 vLLM 默认值。如果设置为文件夹路径，将从指定的文件夹路径加载生成配置。如果生成配置中指定了 `max_new_tokens`，则它将为所有请求设置服务器范围的输出 token 数量限制。
...
:   指定 safetensors 权重的加载策略。
    - "lazy"（默认）：权重从文件内存映射。这支持按需加载，对于本地存储的模型非常高效。
    - "eager"：整个文件在加载前被读取到 CPU 内存中。这推荐用于网络文件系统（例如 Lustre、NFS）上的模型，因为它避免了低效的随机读取，显著加快了模型初始化速度。但会使用更多 CPU 内存。
    - "torchao"：权重被预先加载，然后重建为 torchao 张量子类。这用于检查点使用 torchao 量化并使用 safetensors 保存的情况。需要 torchao >= 0.14.0
...
pt_load_map_location：用于加载 PyTorch 检查点的映射位置，支持只能在特定设备（如 "cuda"）上加载的检查点，这等价于 {"": "cuda"}。另一种支持的格式是从不同设备映射，例如从 GPU 1 到 GPU 0：{"cuda:1": "cuda:0"}。注意，当通过命令行传递时，字典中的字符串需要使用双引号以支持 JSON 解析。更多详情，请参阅 https://pytorch.org/docs/stable/generated/torch.load.html 中 `map_location` 的原始文档。
...
：根据您使用的模型选择推理解析器。用于将推理内容解析为 OpenAI API 格式。
...
：用于分布式模型工作者的后端，可选 "ray" 或 "mp"（多进程）。如果 pipeline_parallel_size 与 tensor_parallel_size 的乘积小于或等于可用 GPU 数量，将使用 "mp" 以保持处理在单个主机上。否则，将引发错误。要使用 "mp"，您还必须设置 nnodes；要使用 "ray"，您必须手动将 distributed_executor_backend 设置为 "ray"。

注意：TPU 仅支持使用 Ray 进行分布式推理。
...
：解码上下文并行组的数量。由于世界大小不会因 dcp 而改变，它只是重用 TP 组的 GPU，因此 tp_size 需要能被 dcp_size 整除。
...
：使用 DCP 或 PCP 时 kv_cache 存储的交错大小。对于 `total_cp_rank = pcp_rank * dcp_world_size + dcp_rank` 和 `total_cp_world_size = pcp_world_size * dcp_world_size`，在 total_cp_rank i 上存储 interleave_size 个 token，然后在 total_cp_rank i+1 上存储下一个 interleave_size 个 token。

- Interleave_size=1：token 级对齐，其中 token `i` 存储在 total_cp_rank `i %% total_cp_world_size` 上。
- Interleave_size=block_size：块级对齐，其中 token 首先填充到前面的 rank。只有当 (rank i, block j) 完全占用后，才会将 token 存储在 (rank i+1, block j) 中。

Block_size 应大于或等于 cp_kv_cache_interleave_size。Block_size 应能被 cp_kv_cache_interleave_size 整除。
...
：是否使用 "hybrid" DP LB 模式。仅适用于在线服务且 data_parallel_size > 0 时。支持在每个节点上运行 AsyncLLM 和 API 服务器，其中 vLLM 在本地数据并行排名之间进行负载均衡，但外部 LB 在 vLLM 节点/副本之间进行负载均衡。需与 --data-parallel-start-rank 显式配合使用。
...
：仅包含解码的批次的双批次重叠阈值。如果请求中的 token 数量大于此阈值，将使用微批处理。否则，请求将在单个批次中处理。

：默认值：`32`

#### `--dbo-prefill-token-threshold`

：包含一个或多个预填充的批次的双批次重叠阈值。如果请求中的 token 数量大于此阈值，将使用微批处理。否则，请求将在单个批次中处理。

：默认值：`512`

#### `--disable-nccl-for-dp-synchronization`, `--no-disable-nccl-for-dp-synchronization`

：强制 vllm/v1/worker/dp_utils.py 中的 dp 同步逻辑使用 Gloo 而非 NCCL 进行 all reduce。
...
:   MoE 层的专家放置策略：
    ...
    - "linear"：专家以连续方式放置。例如，若有 4 个专家和 2 个 rank，则 rank 0 拥有专家 [0, 1]，rank 1 拥有专家 [2, 3]。

    - "round_robin"：专家以轮询方式放置。例如，若有 4 个专家和 2 个 rank，则 rank 0 拥有专家 [0, 2]，rank 1 拥有专家 [1, 3]。此策略有助于改善无冗余专家的分组专家模型的负载均衡。

:   默认值：`linear`

#### `--max-parallel-loading-workers`

:   在分批顺序加载模型时，并行加载工作线程的最大数量。用于避免在使用张量并行和大模型时出现内存溢出（RAM OOM）。

#### `--ray-workers-use-nsight`, `--no-ray-workers-use-nsight`

:   是否使用 nsight 对 Ray 工作线程进行性能分析，参见 https://docs.ray.io/en/latest/ray-observability/user-guides/profiling.html#profiling-nsight-profiler。

:   默认值：`False`

#### `--disable-custom-all-reduce`, `--no-disable-custom-all-reduce`

:   禁用自定义 all-reduce 内核，回退到 NCCL。

:   默认值：`False`

#### `--worker-cls`

:   要使用的工作线程类的完整名称。若为 "auto"，则工作线程类将根据平台自动确定。

:   默认值：`auto`

#### `--worker-extension-cls`

:   要使用的工作线程扩展类的完整名称。工作线程扩展类通过动态继承集成到工作线程类中，用于在 collective_rpc 调用中注入新属性和方法。
...
:   以 token 数量表示的连续缓存块大小。在 CUDA 设备上，仅支持最大为 32 的块大小。

    此配置无静态默认值。若用户未指定，将在 `Platform.check_and_update_config()` 中根据当前平台自动设置。

#### `--gpu-memory-utilization`

:   模型执行器使用的 GPU 内存比例，范围为 0 到 1。例如，值 0.5 表示 50% 的 GPU 内存利用率。若未指定，则使用默认值 0.9。此为每个实例的限制，仅适用于当前 vLLM 实例。在同一 GPU 上运行其他 vLLM 实例不影响此设置。例如，若在同一 GPU 上运行两个 vLLM 实例，可为每个实例分别设置 GPU 内存利用率为 0.5。

:   默认值：`0.9`

#### `--kv-cache-memory-bytes`

:   每个 GPU 的 KV 缓存大小（以字节为单位）。默认情况下，此值为 None，vLLM 可根据 gpu_memory_utilization 自动推断 KV 缓存大小。但用户可能需要手动指定 KV 缓存内存大小。与 gpu_memory_utilization 相比，kv_cache_memory_bytes 提供了更精细的内存使用控制。注意：当 kv_cache_memory_bytes 不为 None 时，将忽略 gpu_memory_utilization。

    支持解析人类可读的整数，如 '1k'、'2M' 等，包括带小数乘数的十进制值。
...
:   KV 缓存存储的数据类型。若为 "auto"，将使用模型的数据类型。CUDA 11.8+ 支持 fp8（=fp8_e4m3）和 fp8_e5m2。ROCM（AMD GPU）支持 fp8（=fp8_e4m3）。Intel Gaudi（HPU）支持 fp8（使用 fp8_inc）。某些模型（如 DeepSeekV3.2）默认使用 fp8，可设置为 bfloat16 以改用 bfloat16；对于不默认使用 fp8 的模型，此选项无效。

:   默认值：`auto`

#### `--num-gpu-blocks-override`

:   要使用的 GPU 块数量。若指定，将覆盖分析得到的 `num_gpu_blocks`。若为 `None`，则不执行任何操作。用于测试抢占。
...
:   设置前缀缓存的哈希算法：
    ...
    - "sha256"：在哈希之前使用 Pickle 进行对象序列化。当前为默认值，因为 SHA256 是避免潜在哈希冲突的最安全选择。

    - "sha256_cbor"：提供可重现、跨语言兼容的哈希。使用规范 CBOR 序列化对象，并用 SHA-256 进行哈希。

    - "xxhash"：使用 Pickle 序列化结合 xxHash（128 位）进行更快的非加密哈希。需要可选的 ``xxhash`` 包。
      重要提示：使用非加密安全的哈希算法理论上会增加哈希冲突的风险，可能导致未定义行为，甚至在多租户环境中泄露私有信息。即使冲突概率极低，也应在启用此功能前权衡安全风险与性能收益。

    - "xxhash_cbor"：结合规范 CBOR 序列化与 xxHash 实现可重现哈希。需要可选的 ``xxhash`` 包。

:   默认值：`sha256`

#### `--cpu-offload-gb`

:   每个 GPU 卸载到 CPU 的空间大小（以 GiB 为单位）。默认值为 0，表示不进行卸载。直观上，此参数可视为虚拟增加 GPU 内存大小的方式。例如，若有一个 24 GB GPU 并设置此值为 10，可虚拟视为一个 34 GB GPU。随后可加载需要至少 26 GB GPU 内存的 13B BF16 权重模型。注意：此功能需要快速的 CPU-GPU 互连，因为模型的一部分需在每次前向传播时从 CPU 内存动态加载到 GPU 内存。

:   默认值：`0`

#### `--calculate-kv-scales`, `--no-calculate-kv-scales`

:   当 kv_cache_dtype 为 fp8 时，启用 `k_scale` 和 `v_scale` 的动态计算。若为 `False`，将从模型检查点加载比例（如果可用）；否则，比例默认为 1.0。

:   默认值：`False`

#### `--kv-sharing-fast-prefill`, `--no-kv-sharing-fast-prefill`

:   此功能正在开发中，当前启用此标志不会触发任何预填充优化。

    在某些 KV 共享设置中（如 YOCO，https://arxiv.org/abs/2405.05254），某些层可跳过对应预填充的 token。此标志允许为符合条件的层覆盖注意力元数据，以实现某些模型（如 Gemma3n）中的此优化所需的元数据。
...
:   用于 Mamba 缓存的数据类型（包括 conv 和 ssm 状态）。若设置为 'auto'，将从模型配置推断数据类型。
...
:   用于 Mamba 缓存的数据类型（仅 ssm 状态，conv 状态仍由 mamba_cache_dtype 控制）。若设置为 'auto'，ssm 状态的数据类型将由 mamba_cache_dtype 决定。

:   默认值：`auto`

#### `--mamba-block-size`

:   Mamba 缓存中连续缓存块的大小（以 token 数量表示）。仅当启用前缀缓存时才可设置。值必须为 8 的倍数，以对齐 causal_conv1d 内核。
...
:   Mamba 层的缓存策略。
    - "none"：当前缀缓存被禁用时设置。
    - "all"：缓存位置 i * block_size 处所有 token 的 mamba 状态。这是当前缀缓存启用时的默认行为（对于支持该功能的模型）。
...
:   用于 KV 缓存卸载的后端。支持的后端包括 'native'（vLLM 原生 CPU 卸载）、'lmcache'。仅当设置了 kv_offloading_size 时，KV 卸载才会被激活。
...
:   如果为 `True`，则启用传递多模态嵌入：
    ...
    对于 OpenAI 兼容服务器，这指的是内容类型为 `"type": "*_embeds"` 的聊天消息。
...
:   要转发给模型处理器的多模态数据参数，例如图像处理器。覆盖从 `transformers.AutoProcessor.from_pretrained` 获得的多模态处理器设置。
...
:   多模态处理器缓存的大小（以 GiB 为单位），用于避免重复处理过去的多模态输入。
    ...
    此缓存在每个 API 进程和引擎核心进程中复制，总内存使用量为
...
:   多模态处理器共享内存缓存中每个对象的大小限制（以 MiB 为单位）。仅当 `mm_processor_cache_type` 为 `"shm"` 时有效。
...
:   指示如何使用张量并行性（TP）优化多模态编码器推理。
    ...
    - `"weights"`：在同一 vLLM 引擎内，将每一层的权重拆分到 TP 等级上。（默认 TP 行为）

    - `"data"`：在同一 vLLM 引擎内，将批处理输入数据拆分到 TP 等级上以并行处理数据，同时在每个 TP 等级上托管完整权重。
...
:   使用视觉变换器时，多模态编码器注意力后端的覆盖选项。接受来自
...
:   启用时，跳过多模态内存分析，仅在引擎初始化期间分析语言骨干模型。
    ...
    这减少了引擎启动时间，但将责任转移给用户，以估算多模态编码器和嵌入缓存激活的峰值内存使用量。
...
:   通过高效视频采样设置视频剪枝率。
    值位于范围 [0;1) 内，决定每个视频中要剪枝的媒体 token 的比例。
...
:   如果为 `True`，将为多模态模型的塔（视觉编码器）和连接器启用 LoRA 支持。这是一项实验性功能，目前仅支持某些多模态模型，例如 Qwen VL 系列。默认为 False。
...
:   默认情况下，只有一半的 LoRA 计算通过张量并行性进行分片。启用此选项将使用完全分片的层。在高序列长度、最大秩或张量并行大小下，这可能会更快。

:   默认值：`False`

#### `--default-mm-loras`

:   将特定模态映射到 LoRA 模型路径的字典；此字段仅适用于多模态模型，当模型在给定模态存在时始终期望 LoRA 处于活动状态时应利用此字段。请注意，目前，如果请求提供多个附加模态，每个模态都有自己的 LoRA，我们不会应用 default_mm_loras，因为我们目前每个提示仅支持一个 LoRA 适配器。在离线模式下运行时，n 个模态的 LoRA ID 将按字母顺序自动分配给 1-n，模态名称按字母顺序排列。
...
启用自指定版本起已被隐藏的已弃用 Prometheus 指标。例如，如果某个先前已弃用的指标自 v0.7.0 版本起已被隐藏，您可以在迁移到新指标期间临时使用 `--show-hidden-metrics-for-version=0.7` 作为应急方案。该指标很可能会在后续版本中彻底移除。

#### `--otlp-traces-endpoint`

:   OpenTelemetry 追踪数据将发送到的目标 URL。

#### `--collect-detailed-traces`

:   可选值：`all`、`model`、`worker`、`None`、`model,worker`、`model,all`、`worker,model`、`worker,all`、`all,model`、`all,worker`

:   仅当设置了 `--otlp-traces-endpoint` 时，设置此参数才有意义。如果设置，将为指定模块收集详细追踪信息。这涉及使用可能代价高昂和/或阻塞的操作，因此可能会对性能产生影响。

    请注意，为每个请求收集详细的时序信息可能会非常耗费资源。
...
:   启用分层 NVTX 追踪。这将对模型中每一层或模块的执行进行追踪，并将输入/输出形状等信息附加到 nvtx 范围标记上。请注意，启用 CUDA 图时此功能无法使用。
...
:   对于分块预填充，可同时进行部分预填充的最大序列数量。

:   默认值：`1`

#### `--max-long-partial-prefills`

:   对于分块预填充，长度超过 long_prefill_token_threshold 的提示可同时进行预填充的最大数量。将此值设置为小于 max_num_partial_prefills 可在某些情况下允许较短的提示优先于较长的提示，从而改善延迟。

:   默认值：`1`

#### `--long-prefill-token-threshold`

:   对于分块预填充，如果提示长度超过此 token 数量，则视为长提示。
...
:   如果设置为 true 且启用了分块预填充，我们不希望部分调度多模态项目。仅在 V1 中使用。
    这可确保如果请求包含混合提示（如文本 token TTTT 后跟图像 token IIIIIIIIII），且只能调度部分图像 token（如 TTTTIIIII，剩余 IIIII），则会分两步调度：第一步调度 TTTT，下一步调度 IIIIIIIIII。

:   默认值：`False`

#### `--scheduler-cls`

:   要使用的调度器类。"vllm.v1.core.sched.scheduler.Scheduler" 是默认调度器。可以直接使用类或类路径，格式为 "mod.custom_class"。
...
:   如果设置为 False，则禁用异步调度。异步调度有助于避免 GPU 利用率出现间隙，从而改善延迟和吞吐量。

#### `--stream-interval`

:   流式传输的间隔（或缓冲区大小），以 token 长度表示。
    较小的值 (1) 通过立即发送每个 token 使流式传输更平滑，
    而较大的值（如 10）通过在发送前批量处理多个 token 来减少主机开销，可能提高吞吐量。
...
VLLMConfig 的 post_init 会进行进一步初始化。如果在 VLLMConfig 外部使用，某些字段将处于不适当的状态。
...
为什么 cudagraph 和 inductor 的大小不同：
- cudagraph：针对特定大小捕获的 cudagraph 只能用于相同的大小。我们需要捕获所有想要使用的大小。
- inductor：由 inductor 为通用形状编译的图可以用于不同的大小。Inductor 也可以针对特定大小进行编译，在这种情况下，它可以获得更多信息，以便使用完全静态的形状来优化图。然而，我们发现通用形状编译在大多数情况下已经足够。针对某些小批量大小进行编译可能是有益的，inductor 擅长优化这些情况。
...
:   最大 cudagraph 捕获大小。
    ...
    如果指定了 cudagraph_capture_sizes，则将其设置为该列表中的最大大小（或检查一致性，如果已指定）。如果未指定 cudagraph_capture_sizes，则大小列表将自动生成，遵循以下模式：

        [1, 2, 4] + list(range(8, 256, 8)) + list(
        range(256, max_cudagraph_capture_size + 1, 16))

    如果未指定，max_cudagraph_capture_size 默认设置为 min(max_num_seqs*2, 512)。这可以避免在 max_num_seqs 较小且内存紧张的情况下出现 OOM，并防止捕获许多大图（>512），这会大大增加启动时间，但性能提升有限。


### VllmConfig

包含所有 vllm 相关配置的 Dataclass。这简化了在代码库中传递不同配置的过程。
